...
function LightBeam.shine(self)
	...
	while emit do
		-- cast until collide
		collision = self:findColl(emit, emitDir)
...
function LightBeam.findColl(self, emit, emitDir)
	...
	if  current.x >= self.parent.rows
		or current.x <= -1
		or current.y >= self.parent.cols
		or current.y <= -1 
	then
		return nil
	end
...
function LightBeam.drawBeam(self, emit, collision, dir)
	...
	local finish = {x = origin.y + cellDim * (collision.y + .5), y = origin.y + cellDim * (collision.y + .5)}
	-- draw
